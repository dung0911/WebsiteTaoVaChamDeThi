 ...
 * @author ADMIN
 */
@WebServlet(name = "EditExamTestServlet", urlPatterns = {"/EditExamTestServlet"})
public class EditExamTestServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditExamTestServlet</title>");
            out.println("</head>");
            out.println("<body>");
            response.setContentType("text/html;charset=UTF-8");
            ...
            book.setExamName(examName);
            book.setExamCode(examCode);
            book.setExamTime(examTime);
            book.setExamDay(examDay);
            book.setScore(score);
            book.setExamDesc(examDesc);
            book.setSubjectId(subjectId);

            UserDatabase bkdao = new UserDatabase(ConnectionPro.getConnection());
            if (bkdao.editTestInfo(book)) {
